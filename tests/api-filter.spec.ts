...
describe('createUserProxy', () => {
  ...
  it('frameVisible host rules', () => {
    ...
    expect(frameVisible(alice, idx, { type: 'host/remote-event', event: 'commands/change', args: [] }, owned)).toBe(true)
    expect(frameVisible(alice, idx, { type: 'host/remote-event', event: 'llm/adapters-updated', args: [] }, owned)).toBe(true)
    expect(frameVisible(alice, idx, { type: 'host/remote-event', event: 'cordis/dynamic-package', args: [] }, owned)).toBe(false)
    expect(frameVisible(alice, idx, { type: 'host/remote-event', event: 'cordis/request-run', args: [] }, owned)).toBe(false)
    expect(frameVisible(root, idx, { type: 'host/remote-event', event: 'cordis/dynamic-package', args: [] }, owned)).toBe(true)
